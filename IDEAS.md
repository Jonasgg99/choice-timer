...
### 2. Show recently-active rooms on the index page (S-M)
Since "Start over" now fully leaves a room (see Shipped notes above), rejoining currently requires the link again or a fresh room. Instead, the setup screen could remember rooms you've recently been in (a small `localStorage` list of room IDs) and offer to rejoin any that are still active — falling back to the plain solo screen if none are, which is the current behavior. Needs a way to check a remembered room is still "live" (e.g. still has a `hostUid` and hasn't gone stale) before offering it, so the list doesn't fill up with dead links.
...
### 8. "Find out more" button — the research behind the method (S)
...
### 13. Setup-time question presets (S)
...
### 16. "Support the creator" button (XS)
...
### 18. Consider renaming the project/repo to "Laterbase" (decision only — rename itself is quick)
