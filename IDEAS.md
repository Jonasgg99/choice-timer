...
Themes requested: better decisions/follow-through, less friction to start, more delight/personality — explicitly *not* return-visit/streak mechanics. "Speak your question" (voice input) was picked from this batch to build immediately — see the Shipped section below and [its design doc](docs/superpowers/specs/2026-08-18-voice-input-design.md). #19 and #20 below are queued as next up after that.

### 15. Gut-check before the timer starts (S) — tried, parked
A private, skippable, never-persisted "which way are you leaning?" tap right after setup, before the countdown is visible — same option buttons, no visible record. The result screen's satisfaction copy then branched on whether the actual outcome matched that private lean. Prototyped on `feature/gut-check` (not merged) — the branch and its commit are left as-is, not deleted, in case it's worth revisiting differently. Verdict: felt like being asked the same question twice in quick succession, rather than the intended "notice your own reaction" moment. If revisited, the framing/timing needs to change, not just the copy — e.g. folding it into an existing step instead of adding a new one, so it doesn't read as a duplicate question.
