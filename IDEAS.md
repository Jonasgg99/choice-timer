...
*In design as of the latest session.* Share a link to a question — from setup and from the live countdown — so a group can decide together (meal plans, movies, trips). After a decision, the group should be able to get a new question without re-sharing a link. This is the current focus; see conversation/design notes for the open architecture question (whether it needs a live backend or can stay link-only). Supersedes the "shareable result as copyable text" idea below, which is now folded into this.
