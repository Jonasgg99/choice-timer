...
- **`emptyAt`'s value isn't bounded against real time.** The rule requires the room to genuinely have zero participants to accept a numeric `emptyAt` write, but doesn't check that the value itself is close to the actual current time — so any signed-in visitor who knows a room's id (a random 6-character code, never guessable without already having the invite link) can stamp an arbitrarily backdated `emptyAt` on it the instant it's transiently empty (e.g. during a host's page refresh) and delete it immediately, bypassing the 30-minute grace period entirely. This is an accepted tradeoff, not an oversight: closing it properly would mean bounding the stamped value to within roughly a minute of Firebase server time, which would also break this project's established fast-forward testing convention (backdating timed fields directly via the SDK instead of waiting out real windows) for this specific mechanism. Consistent with this app's existing "not designed to resist bad actors" trust model (see SHARING-DESIGN.md's "Known simplifications" — e.g. nothing stops a participant from voting twice via a second tab either), and the room id being unguessable without an invite link keeps the practical exploit surface small.
