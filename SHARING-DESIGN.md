...
**Follow-up finding (2026-08-18):** the deadline-passed check above closed the *premature* forgery, but the non-host branches of `state`/`result` still had no check that the writer was actually a current participant of that room — only that they were an authenticated (anonymous) user of the app at all, which every visitor becomes automatically. Combined with `result.answer` never being cross-validated against the room's real `options`, any stranger who merely knew or guessed a roomId could wait for its timer to genuinely expire and then write an arbitrary result to it — a room they had never joined. `messages` already required `participants.child(auth.uid).exists()` for exactly this reason; `state`/`result` were the outliers missing it. Fixed by adding the same participant check to both (every legitimate non-host writer reaches these code paths only via a live room subscription, which by construction means they're already a participant — see `resolveIfExpired`/`maybeStartGracePeriod` in `room.js` — so this closes the gap with no change to legitimate behavior). The residual gap of `result.answer` not being verified against `options` is a smaller data-integrity nicety, not an authorization bypass now that only genuine participants can trigger the write at all — RTDB rules don't cleanly express "must equal one of these N array elements" without restructuring the schema, so it's left as-is.
